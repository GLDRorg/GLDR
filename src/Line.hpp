...
public:
    Line() : vbo(gldr::VertexBuffer::DATA_BUFFER, gldr::VertexBuffer::STATIC_DRAW)
    {
        vao.Bind();
        ...
        {
            auto vs = std::make_shared<gldr::VertexShader>(vert);
            vs->Compile();
            auto s = vs->Status();
            if (!s.empty())
                BREAKPOINT();
        ...
        {
            auto fs = std::make_shared<gldr::FragmentShader>(frag);
            fs->Compile();
            auto s = fs->Status();
            if (!s.empty())
                BREAKPOINT();
